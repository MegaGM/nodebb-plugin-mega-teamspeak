	...
	async.series([
		...
		function (callback) {
			...
			async.filter(user.siteGroups, function (siteGroup, callback) {
				...
				var notMax = _.some(user.siteGroups, function (predicate) {
					if (predicate.weight > siteGroup.weight) return true;
					return false;
				});

				var isGameGroup = _.find(config.ts.gameGroups, { siteGroup: siteGroup.siteGroup });

				if (notMax && !isGameGroup) return callback(false);
			...
			async.each( user.cldbids, function ( cldbid, callback ) {
				methods.getSgidsByCldbid( cldbid, function ( err, res ) {
					var sgids = _.compact( _.filter( res, function ( sgidObj ) {
						...
						return !_.includes( config.ts.transparentSgids, sgidObj.sgid );
					}));
