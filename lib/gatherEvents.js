...
module.exports = function ( data, callback ) {
	...
	var checkOnline = function ( callback ) {
		data.clids = _.map( data.clients, 'clid' );
		if ( !data.clids_old ) data.clids_old = data.clids;

		events.online = _.difference( data.clids, data.clids_old );
		events.offline = _.difference( data.clids_old, data.clids );

		data.clids_old = data.clids;
		callback( null );
	};
	...
	var checkChanges = function ( callback ) {
		data.changes = _.map( data.clients, function ( client ) {
			return {
				clid: client.clid,
				cid: client.cid,
		...
		if ( !data.changes_old ) data.changes_old = data.changes;

		events.changes = _.compact( _.map( data.changes, function ( entry ) {
